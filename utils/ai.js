...
console.log("🔍 Full agent response:\n", JSON.stringify(response, null, 2));
 ...
 //const raw = response.output_text; // ✅ correct property
  ...
  try {
    ...
    // return JSON.parse(jsonString);
        const raw = response.output?.[0]?.content; // ✅ correct path
    if (!raw) {
      console.error("⚠️ No content found in AI response");
      return null;
    }
    console.log("AI Response Raw Content:", raw);
